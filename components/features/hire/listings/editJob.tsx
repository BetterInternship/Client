...
interface EditJobPageProps {
  ...
  // Lets the parent's header know whether the Save button should be
  // disabled, since required-field validation lives here with formData.
  onMissingChange?: (missing: boolean) => void;
}

export interface EditJobPageHandle {
  submit: () => void;
}

const EditJobPage = forwardRef<EditJobPageHandle, EditJobPageProps>(
  function EditJobPage(
    ...
    const isSalaryFilled =
      typeof formData.salary === "number" && formData.salary;
    ...
    const handleSaveEdit = async () => {
      ...
      const edited_job: UpdateJobChallengeListingPayload = {
        ...
        salary_freq:
          formData.allowance === 0 ? formData.salary_freq : undefined,
    ...
    useEffect(() => {
      ...
      setMissing(missing);
      onMissingChange?.(Boolean(missing));
    }, [
      formData.title,
      formData.location,
