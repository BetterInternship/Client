...
export const ApplyConfirmModal = React.memo(
  ({
    job,
    ref,
    onClose,
    ...
    const coverLetterText = useRef<string>("");
    ...
    const hasCL = !!coverLetterText.current.trim();
    ...
    return (
      <ModalComponent ref={ref}>
        <div className="max-w-lg mx-auto p-6 pt-0 overflow-auto">
          ...
          {needsCL && (
            <CoverLetterInput
              setText={(text) => (coverLetterText.current = text)}
            />
          )}
          ...
          <div className="flex gap-3">
            ...
            <Button
              onClick={() => {
                setApplying(true);
                onSubmit(coverLetterText.current)
                  .then(console.log)
