    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-2 sm:gap-3">
      {steps.map((s, idx) => {
        ...
        return (
          ...
          >
            <div className={cn("flex h-8 w-8 sm:h-9 sm:w-9 items-center justify-center rounded-full p-2 shrink-0 ", active ? "bg-primary/10" : "bg-gray-100")}>
              {done ? (
            ...
            <div className="text-xs sm:text-sm font-medium leading-tight min-w-0">
              <div className="text-[10px] sm:text-xs text-gray-400">Step {idx + 1}</div>
 ...
 * @returns 
 */
export const Stepper = ({ steps, onNext, onBack }: { steps: Step[], onNext?: (newStep: number) => void, onBack?: (newStep: number) => void }) => {
	const [step, setStep] = useState(0);
	const currentStep = steps[step];

	return (<div className="w-full mx-auto">
      {step < steps.length && <StepperHeader step={step} steps={steps} />}
