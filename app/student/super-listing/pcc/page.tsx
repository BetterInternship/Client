...
const CHALLENGE_PDF_URL = "https://www.philippinechamber.com/";
const PCC_LOGO_URL =
  "https://www.philippinechamber.com/_next/image?url=%2F_next%2Fstatic%2Fmedia%2Fimg_pcci_logo.35f015b9.png&w=1200&q=75";

type PccSubmissionResponse = {
  success: boolean;
  message?: string;
};

const INITIAL_FORM_STATE: PccSubmissionForm = {
  email: "",
  fullName: "",
  facebookLink: "",
  submissionLink: "",
...
export default function PccSuperListingPage() {
  const isDevelopment = process.env.NODE_ENV === "development";

  const [form, setForm] = useState<PccSubmissionForm>(INITIAL_FORM_STATE);
  ...
  const endpoint = useMemo(() => {
    ...
    if (!base) return "/api/super-listings/submission/pcc";
    return `${base}/super-listings/submission/pcc`;
  }, []);

  const onFieldChange = (field: keyof PccSubmissionForm, value: string) => {
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const data = (await response.json()) as PccSubmissionResponse;
            ...
            <div className="grid gap-8 px-4 py-6 sm:px-6 sm:py-8 lg:grid-cols-[18rem_minmax(0,1fr)] lg:items-start lg:gap-10 lg:px-8">
              ...
              <div className="min-w-0 pt-1 sm:pt-0 lg:border-l lg:border-[#2574BB]/10 lg:pl-10">
                {activePanel === "overview" && (
                  <div id="pcc-overview-anchor" className="w-full">
